#!/usr/bin/env python

"""The setup script."""

from setuptools import setup, find_packages

with open('README.rst') as readme_file:
    readme = readme_file.read()

with open('HISTORY.rst') as history_file:
    history = history_file.read()

requirements = [ ]

test_requirements = [ ]

setup(
    author="Generic and Open Learning Federator",
    author_email='guozh29@mail2.sysu.edu.cn',
    python_requires='>=3.6',
    classifiers=[
        'Development Status :: 2 - Pre-Alpha',
        'Intended Audience :: Developers',
        'License :: OSI Approved :: MIT License',
        'Natural Language :: English',
        'Programming Language :: Python :: 3',
        'Programming Language :: Python :: 3.6',
        'Programming Language :: Python :: 3.7',
        'Programming Language :: Python :: 3.8',
    ],
    description="A scalable, portable, and lightweight Federated Learning framework.",
    install_requires=requirements,
    license="MIT license",
    long_description=readme + '\n\n' + history,
    include_package_data=True,
    keywords='generic_and_open_learning_federator',
    name='generic_and_open_learning_federator',
    packages=find_packages(include=['generic_and_open_learning_federator', 'generic_and_open_learning_federator.*']),
    test_suite='tests',
    tests_require=test_requirements,
    url='https://github.com/MatZaharia/generic_and_open_learning_federator',
    version='0.1.0',
    zip_safe=False,
)
